...
    const response = await fetch('http://localhost:3000/api/user/request-discount-code', {
        method: 'POST',
        headers: {
...
    const response = await fetch('http://localhost:3000/api/user/verify-discount-code', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'x-phone': localStorage.getItem('userAuthToken')
...
        } else {
            shipping = Math.min(subtotal * 0.10, 100);
            shippingText = (shipping > 0) ? `₹${shipping.toFixed(2)}` : "Free";
        }
    } else {
        shippingText = "₹0.00";
    }
