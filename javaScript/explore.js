 ...
 */
function renderAllProducts(wishlist = [], cart = []) {
    const productsContainer = document.getElementById("products-container");
    if (!productsContainer) return;
    
    let productHtml = "";
    allProductsData.forEach(section => {
...
function restoreScrollPosition() {
    ...
    if (savedPos) {
        ...
        sessionStorage.removeItem('exploreScrollPos'); // Clean up so it doesn't persist on manual refresh
    }
}
    ...
    productsContainer.addEventListener('click', async (event) => {
        ...
        for (const section of allProductsData) {
            for (const cat of section.category) {
                const found = cat.items.find(p => p.name === productName);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (userToken) {
        try {
            ...
            localStorage.setItem("cartProducts", JSON.stringify(userData.cart || []));
            localStorage.setItem("viewedItems", JSON.stringify(userData.viewedItems || [])); 
            renderAllProducts(userData.wishlist, userData.cart);
