...
async function renderProductDetails(product, allProductsData, userData = { wishlist: [], cart: [] }) {
    ...
    // --- CHECK FOR DEAL PRICE IN URL ---
    const params = new URLSearchParams(window.location.search);
    const dealPriceParam = params.get('dealPrice');
    
    let displayPrice = product.price;
    let isSpecialDeal = false;

    if (dealPriceParam) {
        displayPrice = parseInt(dealPriceParam);
        isSpecialDeal = true;
    }

    // Calculations based on the DISPLAY PRICE
    const originalPrice = Math.round(displayPrice * (isSpecialDeal ? 1.4 : 1.25)); 
    const discount = Math.round(((originalPrice - displayPrice) / originalPrice) * 100);

    // Update product object price for cart actions
    ...
    const isEligibleForStudentDiscount = displayPrice >= 1000 && 
    ...
    const dealBadgeHtml = isSpecialDeal ? `
        <div class="bg-red-50 text-red-600 text-xs font-bold px-3 py-1 rounded-md inline-block mb-3 border border-red-100">
            <i class="fas fa-tag mr-1"></i> Special Deal Applied
        </div>
    ` : '';
                    ...
                    <div class="lg:col-span-7 p-6 lg:p-8 flex flex-col">
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 mb-6 border border-gray-100">
                            <div class="flex items-end flex-wrap gap-2">
                                <span class="text-3xl font-bold text-gray-900">₹${displayPrice.toLocaleString('en-IN')}</span>
                                <span class="text-gray-400 line-through text-lg">₹${originalPrice.toLocaleString('en-IN')}</span>
                                <span class="text-green-600 font-bold text-sm ml-1">${discount}% OFF</span>
                            </div>
                        </div>
